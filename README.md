...
### 27.18 误以为 GUI 的 `paint` 事件里可以再次提交重绘而不会出事

在 `wingui` 这种最小底层封装里，`paint` 回调应该被当成“系统正在要求你完成这次绘制”，
而不是“再触发一次 `present(...)` 的时机”。

如果在 `paint` 事件处理里再次主动提交重绘，通常会制造重复刷新甚至重绘循环。
更稳妥的做法是：
- 状态变化时调用 `present(...)`
- `paint` 事件本身只当成系统通知，不在里面递归触发新的提交
